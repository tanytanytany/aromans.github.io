...
Если нужно что бы в блоке открывался ТОЛЬКО ОДИН слойлер добавляем  К РОДДИТЕЛЮ ВСЕХ СПОЛЛЕРОВ атрибут data-one-spoller
т.е должно быть  <div class= "имя класса родительского дива" data-spollers data-one-spoller></div>
*/
...
window.onload = function () {
	//Делегирование событий. Выпадающее меню при наведении
	//включаем слушатель для всего документа и запускаем функцию documentActions.
	//documentActions 
	...
	function documentActions(e) {
		...
		//ОТЛАВЛИВАЕМ НАЖАНИЕ НА КНОПКУ "SHOW MORE". ДЛЯ ПОДГРУЗКИ КАРТОЧЕК ИЗ JSON-ФАЙЛА
		if (targetElement.classList.contains('products__more')) {
			getProducts(targetElement);
			e.preventDefault();
		}

		//ОТЛАВЛИВАЕМ НАЖАНИЕ НА КНОПКУ НА КАРТОЧКЕ "ADD TO CART"
		if (targetElement.classList.contains('actions-product__button')) {
			const productId = targetElement.closest('.item-product').dataset.pid;
			addToCart(targetElement, productId);
			e.preventDefault();
		}
		//ОТЛАВЛИВАЕМ НАЖАНИЕ НА ИКОНКУ ДОБАВИТЬ ТОВАР
		if (targetElement.closest('.cart-header__icon') || targetElement.classList.contains('cart-header__icon')) {
		...
		//ОТЛАВЛИВАЕМ НАЖАНИЕ НА КНОПКУ УДАЛЕНИЕ ТОВАРА 
		...
		if (targetElement.classList.contains('checkbox__input')) {
			const filterId = targetElement.closest('.checkbox__input').dataset.filter;
			const checkedElements = document.querySelectorAll('.checkbox__input');
			let filterProducts = document.querySelectorAll('.item-product');


			for (i = 0; i < filterProducts.length; i++) {
				let filterProduct = filterProducts[i];


				if (!filterProduct.classList.contains('f_2') && filterId == 2) {
					filterProduct.classList.add('_hidden');

				} else if (!filterProduct.classList.contains('f_3') && filterId == 3) {
					filterProduct.classList.add('_hidden');

				} else {
					filterProduct.classList.remove('_hidden');
				}
			}
				}
	...
	//ФУНКЦИЯ ЗАКГРУЗКИ БОЛЬШЕГО КОЛИЧЕСТВА ТОВАРОВ НА СТРАНИЦУ
	async function getProducts(button) {
		if (!button.classList.contains('_hold')) {
			button.classList.add('_hold');
			const file = "json/products.json"; //путь к файлу json (обычно указывается url сервера)
			let response = await fetch(file, {
				method: "GET"
			});
			if (response.ok) {
				let result = await response.json(); //Запрос на сервер(здесь )
				loadProducts(result);
				button.classList.remove('_hold');
				//УДАЛЯЕМ САМУ КНОПКУ. В случае, если это настоящий запрос на сервер, то кнопка не удаляется, 
				//а применяем стили, чтобы показать что она неактивна и не допустить посторных нажатий
				button.remove();
			} else {
				alert("Ошибка");
			}
	...
	`;
		...
		<div class="item-product__actions actions-product">
			<div class="actions-product__body">
				<a href="" class="actions-product__button btn btn_item">Add to cart</a>
				<a href="${productShareUrl}" class="actions-product__icon _icon-share">Share</a>
				<a href="${productLikeUrl}" class="actions-product__icon _icon-like">Like</a>
			</div>
		</div>
	`;
	...
	//ФУНКЦИЯ "ПОЛЕТА" ТОВАРА В КОРЗИНУ. 
	...
	//ФУНКЦИЯ ДОБАВЛЕНИЯ ТОВАРА В КОРЗИНУ
		...
		if (productAdd) {
			...
			<a href="" class="cart-list__image cart-list__image_header  _ibg">${cartProductImage}</a>
			<div class="cart-list__body">
				<a href="" class="cart-list__title cart-list__title_header ">${cartProductTitle}</a>
				<div class="cart-list__quantity">Quantity: <span>1</span></div>
				<a href="" class="cart-list__delete cart-list__delete_header _icon-trash"></a>
			</div>
`;
...
if (animItems.length > 0) {
	...
	function animOnScroll() {
		for (let index = 0; index < animItems.length; index++) {
			...
			const animStart = 4;
			// 4 это коэфициент  при преодолении которого добавляется класс _active'
			//т.е. в нашем случае при прокрутке 1/4 высоты объекта, к которому добавляется анимация
	...
	setTimeout(() => {
		...
		var number = document.querySelector('.item-advantage__number'),
